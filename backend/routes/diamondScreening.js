...
router.get("/", authenticateToken, (req, res) => {
  ...
  // Handle filtering by month/year from a DD.MM.YYYY field
  if (liefertermin) {
    const [year, month] = liefertermin.split("-");
    // Filter based on '.MM.YYYY' pattern starting at position 4
    // '15.04.2025' → substr(4, 7) → '04.2025'
    whereClauses.push("SUBSTR(liefertermin, 4, 7) = ?");
  ...
  const where = whereClauses.length
    ? `WHERE ${whereClauses.join(" AND ")}`
    : "";
  const sql = `SELECT * FROM diamond_screenings ${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`;
